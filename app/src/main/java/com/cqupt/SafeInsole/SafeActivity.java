...
public class SafeActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		sButtonGroup = (RadioGroup) findViewById(R.id.sbuttongroup1);
		openButton = (RadioButton) findViewById(R.id.openbutton1);
		closeButton = (RadioButton) findViewById(R.id.closebutton1);
		numberText = (EditText) this.findViewById(R.id.phone_number1);
		contentText = (EditText) this.findViewById(R.id.sent_content1);
		Button button = (Button) this.findViewById(R.id.sent_button1);
		RadioGroupListener listener = new RadioGroupListener();
		sButtonGroup.setOnCheckedChangeListener(listener);
		button.setOnClickListener(new ButtonClickListener());
		util = new SharedPreferencesUtil(this);

		String mykeyvalue = util.read("safekey");
		if (mykeyvalue != null) {
	...
	private final class ButtonClickListener implements View.OnClickListener {
		...
		public void onClick(View v) {
			...
			if(contentText.getText().toString().equals("")){
				...
				Toast.makeText(SafeActivity.this, "保存失败", Toast.LENGTH_SHORT).show();
				return;
			}
			String number = numberText.getText().toString();
			String content = contentText.getText().toString();

			boolean b = util.save("safekey", sButton + "," + number + ","
					+ content);
			String msg;
			if (b) {
				msg = "保存成功";
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		if (keyCode == KeyEvent.KEYCODE_BACK && event.getRepeatCount() == 0) {
			...
			AlertDialog alertDialog = new AlertDialog.Builder(SafeActivity.this)
					.setTitle("退出程序")
					.setMessage("是否退出程序")
